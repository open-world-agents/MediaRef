...
[^1]: The benchmark was conducted against TorchCodec v0.6.0, which only supported `seek_mode=exact` for `.mkv` files, forcing full sequential scans. While `seek_mode=approximate` was added in v0.9.0, TorchCodec still exhibits severe performance issues with `.mkv` files (see [pytorch/torchcodec#1223](https://github.com/pytorch/torchcodec/issues/1223)). MediaRef provides PyAV as a stable alternative.
